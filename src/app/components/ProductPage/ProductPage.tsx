...
const QuantityButton = (quantity: number, handleClick: () => void) => {
...
function ProductPage({
  ...
  const [result, setResult] = useState((price * reduction) / 100);
  ...
  const onClickPicturesContainer = () => {
    setZoomIn(!zoomIn);
    console.log(`delete`);
  };
  ...
  return (
    <>
      {zoomIn && (
        <ZoomIn
          images={PicturesObject}
          onClick={onClickPicturesContainer}
          currentIndex={currentIndexImage}
        />
      )}
      <div className={styles.ProductPage}>
        <Header onClickCart={onClickCart} numberOfItems={cartItems.length} />
